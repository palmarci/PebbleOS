...
- Single BLE connection (`BLE_MAX_CONNECTIONS 1`): phone or pump, never both — toggle between
  them (SPIKE=pump, NORMAL=phone). **v54 changes this to NORMAL⇄DUAL with `BLE_MAX_CONNECTIONS 2`
...
**IMPLEMENTED as a PoC build (v54, 2026-08-30, AWAITING HW) — this section's blockers are the
design; v54 is the first build that addresses all three.** The toggle is now NORMAL⇄DUAL (SPIKE is
...
the phone's). See the v54 entry in `VERSIONS.md` for the full list and HW checklist.
